...
''', (TestProject project) {
      final interfaceItem = project.classNamed('A');
      final item = interfaceItem.allChildren
          .firstWhere((item) => item.element is MethodElement);
      final renderer = project.rendererFor(item);
      final result = renderer.render(GroupType.typeFor(item.element), item);

      expect(
          result, '<pre class="declaration"><code>String foo()</code></pre>');
    });

    testWithSource('handleFunction', r'''
String foo(Object param1) => 'bar: $param1';
''', (TestProject project) {
      final item = project.firstLibrary.allChildren
          .firstWhere((item) => item.element is FunctionElement);
      final renderer = project.rendererFor(item);
      final result = renderer.render(GroupType.typeFor(item.element), item);
    ...
    testWithSource('handleTypeAlias', r'''
...
''', (TestProject project) {
      ...
      final result = renderer.render(
          GroupType.typeFor(interfaceItem.element), interfaceItem);
...
''', (TestProject project) {
      ...
      final renderer = LinkedCodeRenderer(
          project.resolver, project.resolver.fileFor(item.element)!);
    ...
    testWithSource('handleExtension', '''
...
''', (TestProject project) {
    ...
    testWithSource('handleExtensionType', '''
...
''', (TestProject project) {
      ...
      var result =
          renderer.render(GroupType.$extensionType, extensionTypeNamedJSAny);
      ...
      final extensionTypeNamedJSObject = project.extensionTypeNamed('JSObject');
      result =
          renderer.render(GroupType.$extensionType, extensionTypeNamedJSObject);
